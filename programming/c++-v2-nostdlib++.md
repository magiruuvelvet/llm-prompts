...
- The C++ runtime library is not linked (`-nostdlib++`); only header-only standard library facilities are permitted: `<c*>` C wrappers, `<type_traits>`, `<concepts>`, and other purely template/inline headers. Headers requiring runtime linkage (`<string>`, `<vector>`, `<iostream>`, etc.) are prohibited — use C equivalents instead
- `-nostdlib++` removes only the runtime-linked library, not compiler-generated language support. All language features described in this prompt — OOP, virtual dispatch, RAII, destructors, lambdas, templates, and concepts — are emitted directly by the compiler and work correctly without the runtime library
</standard_conformance>

<compiler vendor="clang" flags="-std=c++20 -nostdlib++ -fno-exceptions -fno-rtti" description="implicit compiler options" />
</language_guidelines>
